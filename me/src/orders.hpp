...
public:
    ...
    using price_mov_top_op = std::conditional_t<Side==fun::client::Side::buy, std::plus<Price>, std::minus<Price>>;
    ...
    static constexpr int OB_ARRAY_SIZE = 256;
    static constexpr int init_top_index = 64;
    static constexpr int LOW_ORDER_INDEX_THRESHOLD = OB_ARRAY_SIZE-init_top_index;
    static constexpr price_mov_top_op price_mov_top{};
...
private:
    void maintain_orderbook();
    void pull_orders_from_bottom(int index, Price price);
    ...
    std::array<price_level,OB_ARRAY_SIZE> top_orders_;
    std::map<Price,price_level,price_compare_pred> bottom_orders_;

    Price top_order_price_;
    int top_index_ = init_top_index;
    int top_order_ctr_ = 0;
};
