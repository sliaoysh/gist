    ...
    while (true)
    {
        ...
        switch (msg_type)
        {
            case static_cast<char>(fun::client::MessageType::addOrder):
            {
                ...
                fun::client::Volume volume = add_order->volume;
                const auto ex_order_id = me_.match(add_order->instrumentId, add_order->side, add_order->price, volume, trades_);
                ...
                me_.add_order(ex_order_id, add_order->instrumentId, add_order->side, add_order->price, volume);
                break;
            }
            case static_cast<char>(fun::client::MessageType::cancelOrder):
            {
                ...
                const bool res = me_.cancel_order_check(cancel_order->exchangeOrderId);
                if (res)
                {
                    ...
                    me_.cancel_order(cancel_order->exchangeOrderId);
                }
                else
                {
